...
@login_required(login_url='/login/')
def lista_eventos_geral(request):
    evento = Evento.objects.all()
    dados = {'eventos': evento}
    return render(request, 'agenda/lista_agenda.html', dados)

@login_required(login_url='/login/')
def json_lista_evento(request):
    usuario = request.user
    evento = Evento.objects.filter(usuario=usuario).values('id', 'titulo', 'descricao', 'local_evento', 'data_evento', 'data_final_evento')
    return JsonResponse(list(evento), safe=False)

@login_required(login_url='/login/')
def json_lista_evento_usuario(request, id_usuario):
